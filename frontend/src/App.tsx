...
function App() {
  ...
  if (isDesktopRuntime() && backendStatus === "error") {
    return <main className="login-screen"><div className="login-form"><div className="login-title">MINDCORE</div><p>MindCore could not start.</p><p className="workspace-muted">Diagnostic build: {DIAGNOSTIC_BUILD_LABEL}</p>{startupDiagnostic && <><p className="workspace-muted">Diagnostic Code: {startupDiagnostic.phase}/{startupDiagnostic.category}</p>{startupDiagnostic.operation && <p className="workspace-muted">Operation: {startupDiagnostic.operation}</p>}{startupDiagnostic.lastPhase && <p className="workspace-muted">Last phase: {startupDiagnostic.lastPhase}</p>}{startupDiagnostic.lastOperation && <p className="workspace-muted">Last operation: {startupDiagnostic.lastOperation}</p>}<p className="workspace-muted">Exception: {startupDiagnostic.exceptionClass}</p>{startupDiagnostic.lastPhase && <p className="workspace-muted">MindCore stopped responding while starting this component.</p>}<button className="login-button" type="button" onClick={() => void navigator.clipboard.writeText(startupDiagnostic.line)}>Copy Diagnostic</button></>}<button className="login-button" type="button" onClick={retryDesktopBackend}>Retry</button><button className="login-button" type="button" onClick={() => void invoke("open_configuration_folder")}>Open Configuration</button><p className="workspace-muted">Copy the diagnostic code and send it with the startup report.</p></div></main>;
  }

...
class BootstrapTests(unittest.IsolatedAsyncioTestCase):
 async def test_empty_second_run_and_partial_protection(self):
  ...
  async with p.acquire() as c:
   self.assertEqual(await bootstrap(c),'TURSO_BOOTSTRAP_OK version=23')
   self.assertEqual(await c.fetchval("select value from schema_metadata where key='turso_baseline_version'"),'23')
